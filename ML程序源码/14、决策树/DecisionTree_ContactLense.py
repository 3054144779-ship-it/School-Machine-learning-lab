...
def contact_lense_test():
    """
    desc:预测隐形眼镜的测试代码,并将结果画出来
    args:无
    returns:无
    """
    lenses_data = []
    with open('./3.DecisionTree/lenses.txt') as fr: # 加载隐形眼镜相关的文本文件数据
        lenses_data = [data.strip().split('\t') for data in fr.readlines()] # 解析数据,获得特征数据
        lenses_labels = ['age', 'prescript', 'astigmatic', 'tearRate']  # 得到数据的对应的标签
        lenses_tree = create_tree(lenses_data, lenses_labels)   # 使用上面的创建决策树的代码,构造预测隐形眼镜的决策树
        print(lenses_tree)
        dt_plt.create_plot(lenses_tree) # 画图可视化展示

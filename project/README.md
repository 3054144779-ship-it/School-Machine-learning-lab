...
基于机器学习的高校学生期末成绩预测系统，包含**数据层、算法层、应用层**三层架构。支持个体成绩预测、总体特征分析、决策树模型可视化、在线模型训练，以及学生数据的手动录入与管理。
...
| 模型层 | Python 3 + FastAPI + scikit-learn (Ridge) |
| 数据库 | MySQL 8.0 |
...
### 个体预测
教师通过滑块输入学生 6 项指标（线下互动、期末总成绩、平时成绩、期末成绩、线下期末考试、线下总成绩），系统实时返回预测分数和成绩等级（优/良/中/不及格）。训练管线自动剔除共线特征（如综合平时成绩与期末总成绩完全相关），使用 Ridge 回归避免多重共线性导致的权重失衡。
...
### 模型训练
在线交互式训练界面，支持选择 Excel 文件或数据库两种数据源，自定义选择参与训练的特征列，调节测试集比例（10%-40%）、决策树最大深度（2-15）、随机种子、相关性阈值等超参数。训练完成后展示线性回归和决策树两个模型的完整评估指标（R²/MAE/RMSE/Accuracy/Precision/Recall）、混淆矩阵、特征权重及特征重要性图表，新模型自动热替换上线。

### 历史数据
展示已导入的 518 条学生历史成绩数据，提供统计卡片（总人数、四项均值）和成绩分布柱状图，支持分页浏览完整数据表。支持手动添加单条学生记录和删除记录，数据自动用于数据库训练。
...
| `/api/students` | POST | 手动添加一条学生记录 |
| `/api/students/{id}` | DELETE | 删除指定学生记录 |
...
| 模型 | 指标 | 值 |
|------|------|-----|
| Ridge 回归 (L2) | R² | 0.7509 |
| Ridge 回归 (L2) | MAE | 1.86 |
| Ridge 回归 (L2) | RMSE | 5.26 |
| 分类决策树 | Accuracy | 85.58% |
| 分类决策树 | Precision (weighted) | 0.8446 |
| 分类决策树 | Recall (weighted) | 0.8558 |
